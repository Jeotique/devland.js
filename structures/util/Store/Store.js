...
class Store extends Map {
    ...
    flatMap(fn, thisArg) {
        const Stores = this.map(fn, thisArg);
        return new this.constructor[Symbol.species]().concat(...Stores);
    }
    map(fn, thisArg) {
        if (typeof thisArg !== 'undefined')
            fn = fn.bind(thisArg);
    ...
    reduce(fn, initialValue) {
        ...
        // No items iterated.
        if (first) {
            throw new TypeError('Reduce of empty Store with no initial value');
        }
        return accumulator;
    }
    each(fn, thisArg) {
    ...
    /**
     * Combines this Store with others into a new Store(). None of the source Stores are modified.
    ...
    equals(Store) {
        ...
        for (const [key, value] of this) {
            if (!Store.has(key) || value !== Store.get(key)) {
                return false;
            }
        }
        return true;
     ...
     */
    sort(compareFunction = Store.defaultSort) {
     ...
     */
    sorted(compareFunction = Store.defaultSort) {
